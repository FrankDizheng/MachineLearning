...
def compute_distances(Xtrain, X):
	...
	for j in X:
		lst = []
		for i in Xtrain:
			#row = X[j] col = Xtrain[:i]
			lst.append(np.linalg.norm(j - i))
		dists.append(lst)
	#print(dists)
...
def predict_labels(k, ytrain, dists):
	...
	#####################################################
	ypred = []
	for i in range(dists.shape[0]):	# 0 - num.test - 1
		lst = []
		for j in range(dists.shape[1]):
			lst.append(Freq(dists[i][j], ytrain[j]))
		lst.sort()
		label_list = []
		for m in range(k):
			label_list.append(lst[m].label)
		frequency = {}
		max_frequency = 0
		final_pre = 0
		for e in label_list:
			frequency[e] = frequency.get(e, 0) + 1
			if frequency[e] > max_frequency:
				max_frequency = frequency[e]
				final_pre = e
		ypred.append(final_pre)
