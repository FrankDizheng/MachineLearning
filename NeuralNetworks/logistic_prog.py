...
def logistic_train_ovr(Xtrain, ytrain, w, b, step_size, max_iterations):
    ...
    for i in range(max_iterations):
    	sig = sigmoid(numpy.dot(Xtrain, w) + b)
    	b += (-1) * step_size * (1.0) / n * sum(sig - ytrain)
    	w += numpy.array([(-1) * step_size * (1.0) / n * sum((sig - ytrain) * Xtrain[:,j]) for j in range(d)])
    return w, b
...
def logistic_mul_train(Xtrain, ytrain, w, b, step_size, max_iterations):
    ...
    n, d = Xtrain.shape
    c = ytrain.shape[1]
    ##b_l = numpy.zeros(shape=(n, c))
    for i in range(max_iterations):
    	##for index in range(n):
    	##	b_l[index] = numpy.array(b)
    	w_s = numpy.exp(numpy.dot(Xtrain, w) + b)
    	sums = numpy.dot(w_s, numpy.ones((c, 1)))
    	sums = sums.flatten()
    	p_l = numpy.zeros(shape=(n, c))
    	for j in range(c):
    		p_l[:,j] = w_s[:,j] / sums
    	g = p_l - ytrain
    	for k in range(c):
    		b[k] += (-1) * step_size * (1.0) / n * sum(g[:,k])
    		w[:,k] += numpy.array([(-1) * step_size * (1.0) / n * sum(g[:,k] * Xtrain[:,m]) for m in range(d)]).T
    return w, b
...
def logistic_mul_test(Xtest, w_l, b_l):
    ...
    n, d = Xtest.shape
    d, c = w_l.shape
    ##b = numpy.zeros(shape=(n, c))
    ##for i in range(n):
    ##	b[i] = numpy.array(b_l)
    w_s = numpy.exp(numpy.dot(Xtest, w_l) + b_l)
    sums = numpy.dot(w_s, numpy.ones((c, 1)))
    sums = sums.flatten()
    test_pred = numpy.zeros(shape=(n, c))
    for j in range(c):
    	test_pred[:,j] = w_s[:,j] / sums
    return test_pred
